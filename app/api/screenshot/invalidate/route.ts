...
export async function POST(request: NextRequest) {
  try {
    ...
    if (url && urls) {
      return NextResponse.json({ error: 'Provide either "url" or "urls", not both' }, { status: 400 });
    }
    ...
    if (urls) {
      ...
      for (const u of urls) {
        if (typeof u !== 'string') {
          return NextResponse.json({ error: 'All items in "urls" must be strings' }, { status: 400 });
        }

        try {
          const validUrl = new URL(u);
          if (!['http:', 'https:'].includes(validUrl.protocol)) {
            return NextResponse.json({ error: `URL must use http or https protocol: ${u}` }, { status: 400 });
          }
        } catch (error) {

// Modified by konlyzx (2026) - Added error logging for screenshot API debugging and improved fetch error handling
...
async function captureViaService(url: string, deviceType: 'desktop' | 'mobile' = 'desktop'): Promise<{ screenshot: string; strategy: string }> {
  try {
    ...
    const response = await fetch(apiUrl, {
      ...
      console.error('Screenshot API fetch failed:', err.message);
      throw err;
    })

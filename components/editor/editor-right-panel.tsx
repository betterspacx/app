...
import { backgroundCategories, getAvailableCategories } from '@/lib/r2-backgrounds';
...
export function EditorRightPanel() {
  ...
  const [activeTab, setActiveTab] = React.useState<'presets' | 'settings' | '3d'>('presets');
      ...
      {expanded && (
        <>
          <div className="flex-1 overflow-y-auto p-4">
            <Tabs
              value={activeTab}
              onValueChange={(value) => setActiveTab(value as 'presets' | 'settings' | '3d')}
              className="w-full"
            >
              <TabsList className="w-full grid grid-cols-3 rounded-none bg-transparent h-12 p-1.5 gap-1.5 mb-4">
                <TabsTrigger
                  value="presets"
                  className="data-[state=active]:bg-background rounded-md border-0 data-[state=active]:border-0 transition-all duration-200 text-xs font-medium"
                >
                                ...
                                return (
                                  <div key={category} className="space-y-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-2 overflow-y-auto pb-2 max-h-64">
                                      {categoryBackgrounds.map((imagePath: string, idx: number) => (
                                        <button
                                          key={`${category}-${idx}`}
                                        ...
                                        >
                                          <CanvasThumbnail src={imagePath} className="w-full h-full" />
                                        </button>
                                      ))}
                                    </div>
                                  </div>

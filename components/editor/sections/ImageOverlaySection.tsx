...
function getThumbSrc(overlay: { src: string; isCustom?: boolean }) {
  ...
  return isR2 && !overlay.isCustom ? getR2ImageUrl({ src: overlay.src }) : overlay.src;
}
...
export function ImageOverlaySection() {
  ...
  const totalLayers = imageOverlays.length + textOverlays.length + annotations.length + blurRegions.length;
          ...
          <div className="relative w-10 h-10 shrink-0">
            {previewOverlays.length > 0 ? (
              previewOverlays.map((overlay, i) => (
                ...
                >
                  <img src={getThumbSrc(overlay)} alt="" draggable={false} className="w-full h-full object-contain" />
                </div>
              ))
            ) : (

...
export function AuthModal({ open, onClose }: AuthModalProps) {
  const [user, setUser] = React.useState<{ id: string; email?: string | null; displayName?: string | null; photoUrl?: string | null } | null>(null);
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event: string, session) => {
      if (session?.user) {
        ...
        setUser({
          ...
          displayName: (metadata?.display_name as string) ?? session.user.email?.split('@')[0] ?? null,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (username !== profile?.username) {
        const updated = await updateProfile({ display_name: displayName, username, photo_url: photoURL || null });
    ...
    return (
      ...
      >
        <div className="p-6 space-y-4">
          ...
          {error && <div className="text-xs text-destructive bg-destructive/10 p-2.5 rounded-lg border border-destructive/20">{error}</div>}
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <input type="url" value={photoURL} onChange={(e) => setPhotoURL(e.target.value)} placeholder="https://example.com/avatar.jpg"
                className="w-full h-9 px-3 text-sm rounded-xl border border-border/40 bg-muted/30 text-foreground focus:outline-none focus:ring-1 focus:ring-primary/50" />
            </div>
          </div>
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => setEditing(false)}
              className="flex-1 h-9 text-sm font-medium rounded-xl border border-border/40 bg-muted/30 hover:bg-muted/60 transition-colors text-foreground cursor-pointer">Cancel</button>
  ...
  return (
    ...
    >
      <div className="p-6 space-y-5">
        ...
        <div className="space-y-2">
          <button onClick={() => setEditing(true)}
            ...
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="text-muted-foreground">
              <path d="M17 12a5 5 0 0 1-5 5m-5-5a5 5 0 0 1 5-5" /><path d="M12 7V3m0 18v-4" /><path d="M7 12H3m18 0h-4" />
            </svg>
        ...
        <button onClick={onClose} className="w-full text-xs text-muted-foreground hover:text-foreground transition-colors cursor-pointer">Close</button>
      </div>
    </motion.div>
  );
}
    ...
    <div className="mx-4 rounded-2xl border border-border/40 bg-background shadow-2xl overflow-hidden">
      <div className="flex border-b border-border/40">
        <button onClick={() => setTab('login')}
          className={cn('flex-1 py-3.5 text-sm font-medium transition-colors relative cursor-pointer', tab === 'login' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground')}>
          Sign In
          {tab === 'login' && <motion.div layoutId="auth-tab" className="absolute bottom-0 left-2 right-2 h-0.5 bg-primary rounded-full" />}
        </button>
        <button onClick={() => setTab('signup')}
          className={cn('flex-1 py-3.5 text-sm font-medium transition-colors relative cursor-pointer', tab === 'signup' ? 'text-foreground' : 'text-muted-foreground hover:text-foreground')}>
          Sign Up
          {tab === 'signup' && <motion.div layoutId="auth-tab" className="absolute bottom-0 left-2 right-2 h-0.5 bg-primary rounded-full" />}
        </button>
      </div>
              ...
              <form onSubmit={handleEmailAuth} className="space-y-3">
                ...
                {tab === 'signup' && (
                  <div className="space-y-1.5">
                    ...
                    <Input id="auth-name" type="text" autoComplete="name" placeholder="Your name" value={name}
                      onChange={(e) => setName(e.target.value)} className="h-9 text-sm" required />
                  </div>
                )}
                <div className="space-y-1.5">
                  <Label htmlFor="auth-email" className="text-xs text-muted-foreground">Email</Label>
                  <Input id="auth-email" type="email" autoComplete="email" placeholder="you@example.com" value={email}
                    onChange={(e) => setEmail(e.target.value)} className="h-9 text-sm" required />
                </div>
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <Input id="auth-password" type={showPassword ? 'text' : 'password'}
                      autoComplete={tab === 'login' ? 'current-password' : 'new-password'} value={password}
                      onChange={(e) => setPassword(e.target.value)} className="h-9 text-sm pr-9" required minLength={8} />
                    <button type="button" onClick={() => setShowPassword(!showPassword)}
                      className="absolute right-2 top-1/2 -translate-y-1/2 p-1 text-muted-foreground hover:text-foreground transition-colors cursor-pointer" tabIndex={-1}>
                      {showPassword ? (
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94" />
                          <path d="M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19" /><line x1="1" x2="23" y1="1" y2="23" />
                        </svg>
                      ) : (
                        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">

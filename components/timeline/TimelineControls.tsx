// Modified by konlyzx (2026) - Added margins, rounded corners, and sidebar background to timeline controls
'use client';

import * as React from 'react';
import { PlayIcon, PauseIcon, RepeatIcon, RepeatOffIcon, Add01Icon, Cancel01Icon } from 'hugeicons-react';
...
export function TimelineControls({ onAddAnimation, onClose }: TimelineControlsProps) {
  const { timeline, togglePlayback, setTimeline, setTimelineDuration, clearAnimationClips, animationClips } =
    useImageStore();
  ...
  return (
    <div className="flex items-center px-3 py-2 mx-4 mb-4 bg-sidebar/80 backdrop-blur-xl border-b border-border/30 rounded-lg shrink-0">
      <div className="flex items-center gap-2">
        <button
          onClick={onAddAnimation}
          className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-muted hover:bg-accent transition-colors group cursor-pointer"
        >
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center">
          <span className="text-sm font-semibold text-foreground tabular-nums">{formatTimeDisplay(playhead)}</span>
          <span className="text-sm text-muted-foreground tabular-nums ml-0.5"> / {formatTimeDisplay(duration)}</span>
        </div>

...
export function WebsiteScreenshotInput() {
  ...
  const handleCapture = async () => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Failed to capture screenshot. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div className="w-full space-y-4">
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Enter a website URL to capture a viewport screenshot. Choose desktop (1920x1080) or mobile (375x667) viewport
          size.
        </p>
      </div>

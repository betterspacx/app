...
export function GlobalDropZone({ children }: GlobalDropZoneProps) {
  ...
  const validateFile = React.useCallback((file: File): string | null => {
    if (!ALLOWED_MEDIA_TYPES.includes(file.type)) {
      return 'File type not supported. Use PNG, JPG, WEBP, MP4, WEBM, or MOV.';
    }
  ...
  const handleFiles = React.useCallback(
    (files: File[]) => {
      ...
      const validationError = validateFile(mediaFiles[0]);
      if (validationError) {
      ...
      setTimeout(() => {
        ...
        if (isEditorPage && hasMainImage) {
          // Add as overlay media on top of existing canvas
          ...
          const mediaUrl = URL.createObjectURL(mediaFiles[0]);
          addImages(mediaFiles);
          setScreenshot({ src: mediaUrl });
              ...
              >
                {isProcessing ? (
                  <>
                    ...
                    />
                    <p className="text-sm font-medium text-foreground">Loading media...</p>
                  </>
                ) : (
                  <>
                    <motion.svg
